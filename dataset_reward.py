...
python dataset_reward.py \
  --input_json /root/autodl-tmp/gen_dataset/dataset.json \
  --w_anatomy 1 \
  --w_hps 20
"""
    ...
    def calculate_group_score(self, image_path: str) -> dict:
        ...
        for person in predictions:  
            ...
            for name, indices in self.groups.items():
                ...
                # 低于阈值视为无效部位
                if (name in ['l_feet', 'r_feet']) and (avg_scores >= 3): # 每只脚3个点
    ...
    for item in tqdm(dataset, desc="Scoring Dataset"):
        ...
        for gen_path in item['generated_image_paths']:
            if os.path.exists(gen_path):
                gen_rewards.append({
                    "path": gen_path,
                    "scores": scorer.evaluate(gen_path, prompt)
                })
            else:
                gen_rewards.append({"path": gen_path, "scores": {"total_reward": 0.0, "anatomy_score": 0.0, "hps_score": 0.0}})
        
        # 对生成结果按 reward 降序排序
        gen_rewards.sort(key=lambda x: x['scores']['total_reward'], reverse=True)
        item['gen_rewards'] = gen_rewards

        # 自动标记 Chosen 和 Rejected (用于 DPO)
        # 最好的作为 Chosen，最差的作为 Rejected
        if len(gen_rewards) >= 2:
            item['dpo_pair'] = {
                "chosen": gen_rewards[0]['path'],
                "rejected": gen_rewards[-1]['path'],
                "margin": gen_rewards[0]['scores']['total_reward'] - gen_rewards[-1]['scores']['total_reward']
            }

        processed_data.append(item)

    # 保存结果
    output_path = args.output_json if args.output_json else args.input_json.replace(".json", "_scored.json")
